...
<ArticleService.SelectablePackRow> _rows = new();
        private List
    <ArticleService.SelectablePackRow> _all = new();

        public List
        <(int ArticleId, int? VariantId, decimal Quantity)> SelectedRows
        { get; private set; } = new();
        ...
        private void LoadChoices()
        {
            ...
            <ArticleService.SelectablePackRow>(_all);
        ...
        private void SearchBox_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs e)
        {
            ...
            else
            {
                ...
                _rows = new ObservableCollection
                    <ArticleService.SelectablePackRow>(f);
            }
            GridChoices.ItemsSource = _rows;
        }
        ...
        {
            if (!decimal.TryParse(QtyDefaultBox.Text, NumberStyles.Number, CultureInfo.InvariantCulture, out var q) || q
                        < 1)
            {
            ...
            var picked = GridChoices.SelectedItems.Cast
                            <ArticleService.SelectablePackRow>().ToList();
            if (picked.Count == 0)
            {
